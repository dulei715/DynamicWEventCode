...
def plot_budget_change_for_error(input_basic_path, output_basic_path):
    ...
    for i, dir_name in enumerate(dataset_dirs):
        ...
        output_file_name = os.path.join(
            output_dir,
            f"{output_dataset_names[i]}_budget_change_{metric_name}_containing_error_details"
        )
        bp.plot_budget_change_influence_given_metric_for_single_dataset_containing_error_details(
            abs_dir,
            default_window_size,
            metric_col_index,
            metric_name,
...
def plot_window_size_change_for_error(input_basic_path, output_basic_path):
    ...
    for i, dir_name in enumerate(dataset_dirs):
        ...
        output_file_name = os.path.join(
            output_dir,
            f"{output_dataset_names[i]}_window_size_change_{metric_name}_containing_error_details"
        )
        bp.plot_window_size_change_influence_given_metric_for_single_dataset_containing_error_details(
            abs_dir,
            default_budget,
            metric_col_index,
            metric_name,
...
if __name__ == '__main__':
    ...
    input_basic_error_containing_error_details_path = basic_path + "/3-3_dynamic_stream_pdp/8.result_containing_error_details"
    ...
    plot_window_size_change_for_error(input_basic_error_containing_error_details_path, output_basic_path)