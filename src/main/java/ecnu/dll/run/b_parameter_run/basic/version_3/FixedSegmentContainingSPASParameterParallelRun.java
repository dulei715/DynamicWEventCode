 ...
 * 用于执行
 */
public class FixedSegmentContainingSPASParameterParallelRun implements Runnable {
    ...
    public FixedSegmentContainingSPASParameterParallelRun(String basicPath, String dataTypeFileName, Integer singleBatchSize, Double privacyBudget, Integer windowSize, File[] timeStampDataFiles, int startFileIndex, int endFileIndex, Integer segmentID, CountDownLatch latch, CountDownLatch innerLatch) {
    ...
    private void initialize() {
        ...
        SPAS spas = new SPAS(dataType, privacyBudget, windowSize);
        this.mechanismMap.put(Constant.SPASSchemeName, spas);
...
//        File[] timeStampDataFiles = dirFile.listFiles(new NumberTxtFilter());
        ...
        String basicOutputPathDir = StringUtil.join(ConstantValues.FILE_SPLIT, basicPath, Constant.OutputContainingSPASDirName, "p_"+String.valueOf(privacyBudget).replace(".","-")+"_w_"+windowSize, "segment_"+segmentID);
...
//                System.out.println("Start PersonalizedBudgetAbsorption...");
                ...
                tempResult = _5_SPASMechanismRun.runBatch((SPAS)mechanismMap.get(Constant.SPASSchemeName), batchID, batchDataList, rawPublicationBatchList);
                experimentResultList.add(tempResult);
