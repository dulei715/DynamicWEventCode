 ...
 * 用于执行
 */
public class FixedSegmentShowErrorDetailParameterParallelRun implements Runnable {
    ...
    public FixedSegmentShowErrorDetailParameterParallelRun(String basicPath, String dataTypeFileName, Integer singleBatchSize, Double privacyBudget, Integer windowSize, File[] timeStampDataFiles, int startFileIndex, int endFileIndex, Integer segmentID, CountDownLatch latch, CountDownLatch innerLatch) {
    ...
    public List<ExperimentResult> runSegmentBatch() {
        ...
        String outputDirPath = Constant.OutputContainingErrorDetails;
        String basicOutputPathDir = StringUtil.join(ConstantValues.FILE_SPLIT, basicPath, outputDirPath, "p_"+String.valueOf(privacyBudget).replace(".","-")+"_w_"+windowSize, "segment_"+segmentID);
        ...
        for (int i = startFileIndex; i <= endFileIndex; i++) {
            ...
            if ((i+1) % singleBatchSize == 0 || i == timeStampDataFiles.length - 1) {
                ...
                PurePair<ExperimentResult, List<StreamCountData>> nonPrivacySchemeResultPair = _0_NonPrivacyMechanismRun.runBatchDetails(nonPrivacyMechanism, batchID, batchDataList);
...
//                System.out.println("Start BudgetAbsorption...");
                tempResult = _1_WEventMechanismRun.runBatchDetails((BudgetAbsorption)mechanismMap.get(Constant.BudgetAbsorptionSchemeName), batchID, batchDataList, rawPublicationBatchList);
                experimentResultList.add(tempResult);



//                System.out.println("Start PersonalizedBudgetAbsorption...");
                tempResult = _2_PersonalizedEventMechanismRun.runBatchDetails((PersonalizedBudgetAbsorption)mechanismMap.get(Constant.PersonalizedBudgetAbsorptionSchemeName), batchID, batchDataList, rawPublicationBatchList);
                experimentResultList.add(tempResult);
