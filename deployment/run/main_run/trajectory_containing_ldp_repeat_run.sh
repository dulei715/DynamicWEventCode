...
#maxMem="-Xmx2400g"
for i in {1..10}; do
  java ${totalMem} ${maxMem} -cp DynamicWEventCode-1.0-DMDP-jar-with-dependencies.jar ecnu.dll.run.d_total_run._4_added_run._2_3_containing_ldp_run.${main_java_file_name}ContainingLDPRun
  mkdir -p ${basic_path}/round_${i}_containing_ldp/basic_info
  mv ${basic_path}/basic_info/userTypeID.txt ${basic_path}/round_${i}_containing_ldp/basic_info &
  mv ${basic_path}/basic_info/user_to_type.txt ${basic_path}/round_${i}_containing_ldp/basic_info &
