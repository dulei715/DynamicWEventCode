...
#maxMem="-Xmx2400g"
for i in {1..10}; do
  java ${totalMem} ${maxMem} -cp DynamicWEventCode-1.0-DMDP-jar-with-dependencies.jar ecnu.dll.run.d_total_run._4_added_run._2_4_containing_spas_run.${dataset_dir_name}ContainingSPASRun
  mkdir -p ${basic_path}/round_${i}_containing_spas
  mv ${basic_path}/basic_info ${basic_path}/round_${i}_containing_spas &
  mv ${basic_path}/runInput ${basic_path}/round_${i}_containing_spas &
