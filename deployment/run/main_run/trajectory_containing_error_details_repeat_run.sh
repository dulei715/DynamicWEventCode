...
#maxMem="-Xmx2400g"
for i in {1..10}; do
  java ${totalMem} ${maxMem} -cp DynamicWEventCode-1.0-DMDP-jar-with-dependencies.jar ecnu.dll.run.d_total_run._4_added_run._2_6_containing_error_details_run.${main_java_file_name}ContainingErrorDetailsRun
  mkdir -p ${basic_path}/round_${i}_containing_error_details/basic_info
  mv ${basic_path}/basic_info/userTypeID.txt ${basic_path}/round_${i}_containing_error_details/basic_info &
  mv ${basic_path}/basic_info/user_to_type.txt ${basic_path}/round_${i}_containing_error_details/basic_info &
