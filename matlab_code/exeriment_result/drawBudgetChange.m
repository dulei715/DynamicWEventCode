...
for temp_name = dir_names
    ...
    % y_ba(i) = temp_table(3,8).MRE;
    % y_pbd(i) = temp_table(4,8).MRE;
    % y_pba(i) = temp_table(5,8).MRE;
    % y_pdbd(i) = temp_table(6,8).MRE;
    % y_pdba(i) = temp_table(7,8).MRE;
...
plot(x, y_ba, 'mx-', 'LineWidth', 2, 'MarkerSize',figure_MarkerSize);
plot(x, y_pbd, 'b+-','LineWidth',2, 'MarkerSize',figure_MarkerSize);
plot(x, y_pba, 'ro-','LineWidth',2, 'MarkerSize',figure_MarkerSize);
plot(x, y_pdbd, 'cd-','LineWidth',2, 'MarkerSize',figure_MarkerSize);
plot(x, y_pdba, 'gd-','LineWidth',2, 'MarkerSize',figure_MarkerSize);
ylabel(yLabelName);
