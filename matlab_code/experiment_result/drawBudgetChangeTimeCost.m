...
for dir_name = dataset_dirs
    ...
    outputFileName = fullfile(char(output_basicPath), 'fig', 'experiment_result', char(strcat(output_dataset_names(i),"_budget_change_running_time.eps")));
    %drawBudgetChangeForSingleDatset(abs_dir, default_window_size, outputFileName);
    drawBudgetChangeOnlyForTimeCostSingleDataset(abs_dir, default_window_size, outputFileName);
end